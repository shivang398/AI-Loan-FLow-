...
export const CibilCheckPage: React.FC = () => {
  ...
  const handleCibilCheck = async (values: any) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Failed to fetch CIBIL data. Please try again.');
    } finally {
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <div className="page-header">
        ...
        <h1 className="page-header-title">CIBIL Soft Pull</h1>
        <span className="page-header-subtitle">
          Zero-impact institutional credit check via secure API &ensp;
