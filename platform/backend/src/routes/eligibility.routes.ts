...
router.post('/cibil-bureau/check', cibilLimiter, async (req: Request, res: Response) => {
  ...
  const score = 640 + Math.floor((parseInt(mobileNumber?.slice(-3) ?? '0') % 211));
  const demoResult = {
    demoMode: true,
    cibilScore: score,
    scoreBand: scoreBandFromScore(score),
...
// POST /cibil-bureau/report — ADMIN only — same PDF layout as CRIF report
router.post('/cibil-bureau/report', requireRoles('ADMIN'), async (req: Request, res: Response) => {
  ...
  doc.fillColor('#93C5FD').font('Helvetica').fontSize(7)
     .text('Data sourced from TransUnion CIBIL Limited (RBI Licensed CIC) via Tenacio Analytics Pvt. Ltd.  |  Soft Pull — No enquiry footprint',
           MARGIN, y + 5, { width: COL, lineBreak: false });
  y += 24;

  const leftW = 305; const rightW = COL - leftW - 8; const rx = MARGIN + leftW + 8; const boxH = 128;
  ...
  const rows = [
    ['Full Name', clip(fullName, 35)], ['Mobile', mobile || '—'],
    ['Date of Birth', r.dob || '—'], ['Occupation', clip(r.occupationType || '—', 30)],
    ['Net Income', clip(r.income || '—', 30)], ['Address', clip(r.address || '—', 48)],
  ];
  let py = y + 26;
  ...
  doc.fillColor(NAVY).font('Helvetica-Bold').fontSize(7.5).text('CIBIL CREDIT SCORE', rx + 10, y + 9, { lineBreak: false });
  ...
  doc.fillColor(GREY).font('Helvetica').fontSize(6)
     .text('TransUnion CIBIL v2', rx, y + 107, { width: rightW, align: 'center', lineBreak: false });
  y += boxH + 8;
  ...
  const halfW = (COL - 6) / 2; const hasOverdue = (r.totalOverdue ?? 0) > 0;
  ...
  doc.fillColor(hasOverdue ? RED : GREEN).font('Helvetica-Bold').fontSize(16)
     .text(hasOverdue ? '₹' + fmtINR(r.totalOverdue) : 'NIL', MARGIN + halfW + 16, y + 20, { width: halfW - 20, lineBreak: false });
  doc.fillColor(GREY).font('Helvetica').fontSize(6.5)
  ...
  const metaItems = [
    ['ENQUIRIES (6M)', String(r.enquiryCount ?? 0)], ['REPORT ID', clip(reportId, 18)],
    ['SCORE DATE', r.scoreDate ?? now.split(',')[0]], ['BUREAU', 'TransUnion CIBIL'], ['PULL TYPE', 'Soft Pull'],
  ];
  ...
  doc.fillColor(GOLD).font('Helvetica').fontSize(7).text('CIBIL Bureau Report  —  CREDIT ACCOUNT DETAILS', MARGIN + 170, 10, { lineBreak: false });
  doc.fillColor('#94A3B8').font('Helvetica').fontSize(7)
     .text(clip(fullName, 30) + '  |  ' + mobile, W - MARGIN - 200, 10, { width: 200, align: 'right', lineBreak: false });
  y = 40;

  const cols = [
    { label: 'LENDER', w: 112, maxCh: 17 }, { label: 'TYPE', w: 100, maxCh: 20 },
    { label: 'ACCOUNT NO.', w: 103, maxCh: 19 }, { label: 'OPENED', w: 48, maxCh: 10 },
    { label: 'BALANCE (₹)', w: 70, maxCh: 13 }, { label: 'OVERDUE (₹)', w: 60, maxCh: 12 },
  ...
  for (let i = 0; i < accounts.length; i++) {
    if (y + rowH > PAGE_SAFE) {
      ...
      y = 40; y = drawTableHeader(y);
    }
    const acct = accounts[i]; const isOverdue = (acct.amountOverdue ?? 0) > 0; const isClosed = !!acct.dateClosed;
    ...
    for (let j = 0; j < cells.length; j++) {
      doc.fillColor(cells[j].color).font(j === 6 ? 'Helvetica-Bold' : 'Helvetica').fontSize(6.5)
